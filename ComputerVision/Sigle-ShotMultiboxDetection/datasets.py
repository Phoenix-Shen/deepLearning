...
def read_data_bananas(data_root, is_train=True):
    ...
    for img_name, target  in csv_data.iterrows():
        images.append(
            tv.io.read_image(
                os.path.join(data_root,"bananas_train" if is_train else "bananas_val","images",f"{img_name}")
            )
        )
...
class BananasDataset(t.utils.data.Dataset):
    ...
    def __init__(self,data_root,is_train=True) -> None:
        super().__init__()
        self.features,self.labels = read_data_bananas(data_root=data_root,is_train=is_train)
        print("read"+ str(len(self.features))+ ("training examples" if is_train else "validation_samples"))
...
if __name__ == "__main__":
    batch_size =32
    train_iter ,_= load_data_bananas(r"../../dataset\banana-detection",batch_size)
